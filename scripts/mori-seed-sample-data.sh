...
if [ -f "$PROJECT_ROOT/schema/005_alert_resolved.sql" ]; then
  run_sql_file "$PROJECT_ROOT/schema/005_alert_resolved.sql" >/dev/null 2>&1 || true
fi
if [ -f "$PROJECT_ROOT/schema/006_evidence_events.sql" ]; then
  run_sql_file "$PROJECT_ROOT/schema/006_evidence_events.sql" >/dev/null 2>&1 || true
fi
if [ -f "$PROJECT_ROOT/schema/007_controls.sql" ]; then
  run_sql_file "$PROJECT_ROOT/schema/007_controls.sql" >/dev/null 2>&1 || true
fi
if [ -f "$PROJECT_ROOT/schema/008_settings.sql" ]; then
  run_sql_file "$PROJECT_ROOT/schema/008_settings.sql" >/dev/null 2>&1 || true
fi
