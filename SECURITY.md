...
| **Audit log** | Action audit log is **hash-chained** and **persisted to PostgreSQL** (append-only `ui_audit_log`; the chain continues across restarts). `GET /admin/audit-log/verify` recomputes the full DB chain and reports tampering/deletion. No update/delete API. External log forwarding + signed daily-root export remain backlog. |
