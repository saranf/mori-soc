...
- ✅ **통제 클릭 → 라이브 실증적 + 증적 팩 PDF(1클릭)** — 트리에서 통제를 열면 소스별 **실데이터 직결**(예: 2.11.2 → Trivy 미조치 CVE 실건수, A.8.16 → 최근 7일 탐지·모니터링 자산, 계정통제 → MORI 인시던트/감사). `GET /controls/detail/{id}`(+`/evidence.pdf`) — 매핑·관련 결함·**현재 증적 공백 수(live)**까지. 클릭 시 해당 탭 딥링크
- ✅ 대시보드 **GRC 프리셋** — 오늘의 작업 큐(증적 공백) 카드(admin·security) · **카탈로그 CI**(검증 + JSON 신선도, GitHub Actions)
- 🔲 남은 일: draft 통제 계속 채우기(커버리지 ↑) · 소스별 라이브 조인 정밀화(호스트↔통제 단위)
