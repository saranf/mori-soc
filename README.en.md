...
- 🌐 **Multi-language UI** — Korean / English toggle on every page (login, dashboard, admin console); the selected language is persisted in a cookie + localStorage and switches instantly without a reload
...
flowchart LR
    ...
    subgraph REPO["Repositories"]
        ...
        STR["UI operational state (in-memory)<br/>asset_owners / asset_audit_log<br/>vuln_actions / triage_store / incident_store"]
    end
...
> Solid lines = current operating flow. PostgreSQL holds **normalized seed data** (hosts/alerts/vulns/observations) which is loaded into InMemoryRepository at boot for queries. The 5 UI operational stores (triage / incidents / asset owners / vuln actions / asset audit log) currently run in-memory; the **dashed line = next milestone (Postgres persistence + activating real-time pollers)**.
...
| **🌐 Multi-language (KO/EN)** | Top-right toggle on login, dashboard, and admin pages + `data-i18n` static substitution + `window.t()` dynamic messages | Persisted in cookie / localStorage; active tab re-renders instantly on toggle |
| **API docs** | Swagger `/docs` | Auto-generated by FastAPI |

> ⚠️ **Storage separation notice** — PostgreSQL holds **normalized seed security data** (hosts/alerts/vulnerabilities/observations etc.), loaded into InMemoryRepository at boot for queries. Meanwhile, the **5 UI operational state stores** (triage / incidents / asset owners / vuln actions / asset audit log) are currently in-memory dicts within the API process, so they **reset on restart**. Postgres persistence mapping is prepared in code/schema at `repositories/postgres.py` + `schema/002_phase2_compliance_identity.sql`; only the actual connection wiring is pending.

### 🟡 In progress / Next steps (next milestone)

| Item | Status | Priority |
|---|---|---|
| **UI operational state → PostgreSQL persistence** | `repositories/postgres.py` + `schema/002_*.sql` ready. 5-store mapping not yet connected | 🔴 High |
...
| **UI operational state — 5 in-memory stores** (reset on restart) | Module-scope dicts in API process. Postgres persistence not yet connected | `api/server.py` |
| Phase 2 persistence (5 stores → Postgres) | 🔲 Planned — mapping code/schema ready in `schema/002_phase2_compliance_identity.sql` + `repositories/postgres.py` | — |

#### In-memory 5-store detail
...
→ Phase 2's next milestone is to **map the 5 stores above to PostgreSQL tables** for persistence.
...
| Triage / asset owners / vulnerability actions / incident changes | 0 | Accumulate as you edit in the UI |
...
3. **Postgres persistence + REST poller activation** — Replace seeds with live data from the integrations above (includes persistence of the 5 UI operational state stores)
...
1. **PostgreSQL persistence** — Map the 5 in-memory stores (`asset_owners`, `asset_audit_log`, `vuln_actions`, `triage_store`, `incident_store`) to `repositories/postgres.py`.
...
| PostgreSQL — UI operational state 5-store persistence (Phase 2) | 🔲 Incomplete (`schema/002_*.sql` + `repositories/postgres.py` mapping prepared) |
