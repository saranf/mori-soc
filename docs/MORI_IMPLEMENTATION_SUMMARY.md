...
MORI는 SOC-lite 배포 스캐폴드에서 출발해, **데이터 수집/정규화/질의 코어 (Phase 1)** + **운영 + 감사 증적 UI (Phase 2 Alpha)** 단계까지 진행됐습니다. 지향점은 **ISMS-P / ISO 27001 인증 심사를 일상 운영과 같이 끌고 갈 수 있는 Audit-Ready Compliance-Evidence Platform** 입니다.

| 단계 | 상태 | 핵심 산출물 |
| --- | --- | --- |
| Phase 1 — 데이터 수집/정규화 코어 | ✅ 완료 | collectors / normalization / ingestion / 12 intent query catalog |
| MVC 1 — FastAPI HTTP API | ✅ 완료 | `/health` `/catalog` `/query` `/interpret` `/dashboard/summary` |
| MVC 2 — PostgresRepository | ✅ 코드 완료 / 🔲 운영 미연결 | `repositories/postgres.py` (인메모리에서 운영 중) |
| MVC 3 — Docker/Compose 배포선 | ✅ 완료 | `docker-compose.yml`, GitHub Actions |
| MVC 4 — 자연어 질의 + 운영 UI | ✅ 완료 | `intent_parser` + `/ui` |
| **Phase 2 Alpha — 운영 UI + 감사 증적** | ✅ Alpha 운영 중 | RBAC, 자산/취약점/Triage/인시던트/PDCA, 감사 로그, 5종 증적 리포트 |
| Phase 2 — 영속화 + 실시간 폴링 | 🔲 다음 작업 | 인메모리 store → Postgres, 폴러 연결 |
| Phase 3 — 조사형 멀티 피벗 에이전트 | 🔲 미착수 | host/user/ip 다단계 pivot |

---

## 2. Phase 2 Alpha 에서 추가된 핵심 기능

### 인증 / 권한

- 로그인 / 세션 / 가입 요청·승인 흐름
- RBAC (admin / security / moniter) — 역할별 탭/기능 on·off
...
3. **collector lag / sync 상태 표시** — 대시보드에 source freshness 카드 추가

### 🟡 운영 안정성

4. **LDAP 인증 운영 적용** — `LDAP_URL` 환경변수 + AD/LDAP 검증
5. **HTTPS / 리버스 프록시** — Nginx/Caddy + TLS
6. **PDF 증적 리포트** — CSV 미리보기에 PDF 출력 추가

### 🟢 기능 확장

7. **Trivy 결과 자동 적재** — 온디맨드 스캔 결과를 ingestion 파이프라인에 연결
8. **SQL 기반 읽기 최적화** — snapshot 조회를 Postgres view 기반으로 점진 전환
9. **Phase 3 — 조사형 multi-hop pivot 에이전트**

---

## 7. 한 줄 정리

MORI는 이제 **"보안 데이터를 모으고, 한눈에 보고, 자연어로 질의하고, 변경을 감사 증적으로 누적하는" Audit-Ready 운영 플랫폼 Alpha 단계**입니다. 다음 핵심은 **인메모리 store를 Postgres로 영속화하고 실시간 폴링을 연결하는 것**입니다.
