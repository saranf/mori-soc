...
풀 인증범위 관리 모듈(조직도·사업장·계약)이 아니라, 자산에 **범위 태그**만 붙이고 **인증범위
자산 중 기술 신호(모니터링)로 커버되는 비율**을 보여준다(모리다움 — 작게, 증적 중심).

- `services/scope_coverage.py`의 `compute_scope_coverage(owners, monitored_hosts)`는 순수 함수로
  태그별 assets/monitored/coverage_pct + 인증범위(`인증범위 포함`) 커버리지를 계산.
- 자산 담당자 레코드에 `scope_tags`(정규화 리스트) 필드 추가(단건 upsert·CSV import 공통).
