...
| # | 항목 | 유형 | 규모 | 안전도 |
|---|---|---|---|---|
| F1 | **요청 스코프 스냅샷 메모이즈** — 한 요청 내 `get_query_service()` 재호출이 같은 스냅샷 재사용(요청 끝나면 폐기) | 성능·정확성 유지 | 중 | 높음(상태무변경) |
| F2 | (F1 이후) 폴링용 **짧은 TTL(2~5s) 스냅샷 캐시** — 대시보드 반복 폴을 흡수, 쓰기 시 무효화 | 성능 | 중 | 중(짧은 staleness) |
| F3 | **호스트 vs 아티팩트 분류 단일화** — 이미지 아티팩트를 host 카운트에서 일관 제외/표시 | 정합성 | 소 | 높음 |
| F4 | M10 Phase A(세션 Postgres) — 위 설계대로 플래그 옵트인 | 견고성 | 대 | 중(코어) |

**착수 순서 추천:** F3(작고 안전·혼란 제거) → F1(스냅샷 메모이즈, 대시보드 부하↓) → F4(M10 세션) → F2 → M10 B/C.
F1·F3 은 안전한 즉시 개선, F4(세션)·F2 는 코어라 설계 검토 후 신중히.
