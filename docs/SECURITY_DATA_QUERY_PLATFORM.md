...
| **Phase 2 — 영속화 + read-only 소스 통합 + 실시간 폴링** | 🟡 진행 중 | 6종 운영 store → Postgres ✅ 완료(M2-1), config 기반 read-only 소스 온보딩(N-series) 🔲, 폴러 활성화 🔲 |
...
5. **감사 증적 PDF 출력** — 현재 5종 CSV 미리보기에 PDF 옵션 추가
